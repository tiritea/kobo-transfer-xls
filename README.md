...
### Ona: repeats within groups
The tool additionally supports editing or creating - or migrating - forms that have repeat groups. Repeat group responses are exported by Kobo into the XLSX spreadsheet as separate *sheets* - one sheet per repeat - that contain the question columns and responses within that repeat. These are further linked to their parent group (or sheet) via a **_parent_index** field. In this way the tool is able to reconstruct the correct submission XML hierarchy containing the repeat responses for a successful OpenRosa POST submission.

Repeats in Ona are exported in a similar fashion; however, Ona prefixes the sheet name created for each repeat also *with the name of the parent group* (or repeat), whereas Kobo doesn't prefix the sheet names with anything. This causes the tool to be unable to correctly determine the correct repeat group name to use for the resulting submission XML document. Unfortunately, Ona further uses an underscore '_' as the separator between this group prefix and repeat name. But because underscores can also legitimately be used within a (repeat) XML element name itself, it is not programmatically possible to reliably distinguish what is the group prefix from what is the repeat name by just looking at the sheet name string.

Therefore, if your Ona XLSForm contains repeats *and these repeats are within a group* (or another repeat) then you will have to manually edit the Ona-exported XLSX file to strip the sheet name for repeats of any group prefix, so that each additional repeat sheet name in the XLSX is just the name of the repeat. This should only be necessary if the repeat is inside a group, which the XLSForm definition will tell you.

### Ona: submission attachments (eg images)
As described above, the tool can include submission attachments - including photos and other images - when creating new submissions in Kobo, but only if these attachments are 
in suitably name submission-specific subdirectories. When you export the media attachments for a project in Kobo, the resulting zip file will expand to submission-specific subdirectories, each subdir containing the attachments/photos that were uploaded for the submission. Unfortunately, when you similarly export the media attachments for a project in Ona, the resulting zip file expands to a *single* directory containing *all* the attachments, thereby disassociating the files from their respective submission.

In order to include the correct attachment(s) for each submission when re-submitting back into Kobo, it is necessary to rename all the Ona-exported media attachment files back into their submission-specific subdirectories. This is the purpose of the helper script `helpers/rename_ona_media.py`. When you run this script from the directory that contains all the unzipped attachment files from the Ona media export, it will call the Ona API to determine which attachment file(s) is associated with which submission, and put it in a submission-specific subdirectory named after the *index* corresponding of the submission as listed in the Ona data export XLSX spreadsheet. These submission attachments will now be in the correct subdir structure to be picked up and included when creating new submissions in Kobo; see https://github.com/tiritea/kobo-transfer-xls/tree/main/xls_transfer#media-upload.

Note - presently submission attachments have to be under a `./attachments` subdir from where you run the tool, which is further the kobo-transfer-xls (sub)directory.
